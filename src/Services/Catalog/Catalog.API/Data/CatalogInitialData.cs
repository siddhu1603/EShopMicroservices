...
namespace Catalog.API.Data
{
    ...
    {
        public async Task Populate(IDocumentStore store, CancellationToken cancellation)  //populates the DB with seed data
        {
            using var session = store.LightweightSession();

            if (await session.Query<Product>().AnyAsync())                                //check if there already exists some data in the DB then no need of seeding
                return;
