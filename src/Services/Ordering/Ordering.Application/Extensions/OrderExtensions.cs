...
{
    public static IEnumerable<OrderDto> ToOrderDtoList(this IEnumerable<Order> orders)  //using the this keyword for IEnumerable<Orders> makes it an extension method for IEnumerable<Orders> type of var.
    {
        return orders.Select(order => new OrderDto(  //for each order in orders we are creating a new orderdto, using the select statement.
            Id: order.Id.Value,
